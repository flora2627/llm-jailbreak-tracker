#!/usr/bin/env python3
"""从 data/*.json + template.html 生成 index.html(LLM 越狱立场图)。
...
def check():
    ...
    for p in papers:
        ...
        if p["kind"] not in ("paper", "cluster", "program", "competition", "blogpost"):
...
# ------------------------------------------------------------------ 渲染资料表
KIND_CN = {"paper": None, "cluster": "论文簇", "program": "项目",
           "competition": "竞赛", "blogpost": "博客"}
...
for p in sorted(papers, key=lambda p: (p["year"], p["lane"], p["label"])):
    ...
    link = (f'<a href="{p["url"]}">{p["arxiv"] or "链接"}</a>' if p.get("url") else
            f'<span style="color:var(--muted)">{html.escape(p["venue"] or "—")}</span>')
...
aria = (f"LLM 越狱研究领域 2017 至 2026 的时间线森林图:{len(lanes)} 条研究主干泳道上分布 "
        f"{len(papers)} 个条目,以 {len(edges)} 条签名边连接——灰实线为继承 {et['inherit']} 条、"
        f"青色佐证 {et['support']} 条、琥珀虚线反驳证伪 {et['refute']} 条。"
        f"反驳边约为佐证边的 {et['refute']/max(et['support'],1):.1f} 倍。"
        f"泳道从上到下依次是对抗根系、对齐训练、离散优化攻击、语义与黑盒攻击、分布偏移与多轮、"
        f"微调与权重篡改、推理期防御、训练期防御、评测基建、机制可解释性、理论天花板。"
